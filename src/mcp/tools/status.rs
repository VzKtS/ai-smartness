...
pub fn handle_help(
    ...
    Ok(serde_json::json!({
        ...
        "categories": [
            ...
            "Thread Lifecycle (ai_thread_create, ai_thread_rm, ai_thread_list, ...)",
            "Thread Operations (ai_merge, ai_split, ...)",
            "Thread Metadata (ai_label, ai_rename, ai_rate_importance, ...)",
            "Bridges (ai_bridges, ai_bridge_analysis, ...)",
            "Focus & Pins (ai_focus, ai_unfocus, ai_pin)",
            "Cognitive Messaging (ai_msg_focus, ai_msg_ack)",
            "Shared Cognition (ai_share, ai_discover, ai_subscribe, ...)",
            "System & Status (ai_status, ai_sysinfo, ai_help, ai_suggestions)",
            "Maintenance (ai_cleanup, ai_lock, ai_backup)",
            "Agent Management (agent_list, agent_query, agent_configure, ...)",
            "Inter-Agent Messaging (msg_send, msg_inbox, msg_reply, ...)",
        ],
    }))
}
