...
use ai_smartness::config::GossipConfig;
...
pub fn handle_thread_create(
    ...
    // Continuity: ask daemon for current PendingContext (tracks the true last
    // thread in the continuity chain, including other MCP-created threads).
    // Falls back to DB query if daemon is unreachable.
    let continuity_parent_id: Option<String> = daemon_ipc_client::send_method(
        "get_pending_context",
        serde_json::json!({
    ...
    .ok()
    .and_then(|v| v.get("thread_id").and_then(|t| t.as_str()).map(String::from))
    .or_else(|| {
        // Fallback: DB query if daemon unreachable
        ctx.agent_conn
            .query_row(
                "SELECT id FROM threads WHERE status IN ('Active', 'Suspended') \
    ...
    // Send to daemon for full pipeline processing (LLM extraction, concepts,
    // thinkbridges, embedding) — same treatment as all other threads.
    let _ = daemon_ipc_client::send_method("enrich_thread", serde_json::json!({
        "project_hash": ctx.project_hash,
        "agent_id": ctx.agent_id,
        "thread_id": thread_id,
        "content": content,
    ...
    // Update daemon's PendingContext so the next automatic capture
    // (prompt/response/tool) chains continuity to this MCP-created thread.
    let _ = daemon_ipc_client::send_method("update_pending_context", serde_json::json!({
        "project_hash": ctx.project_hash,
        "agent_id": ctx.agent_id,
        "thread_id": thread_id,
        "title": title,
    ...
    // __mind__ specific: coherence chain + nanobeat savepoint
    if thread.tags.contains(&"__mind__".to_string()) {
        ...
        let agent_data = ai_smartness::storage::path_utils::agent_data_dir(ctx.project_hash, ctx.agent_id);
        let mut beat = ai_smartness::storage::beat::BeatState::load(&agent_data);
        beat.record_mind_savepoint();
...
pub fn handle_label(
    ...
    match mode.as_str() {
        ...
        "set" | "add" | "remove" => {
            ...
            // Refresh thinkbridges: labels changed → invalidate + re-create shard bridges
            let gossip_cfg = GossipConfig::default();
            let (deleted, created) = ThreadManager::refresh_thinkbridges(
                ctx.agent_conn,
                &id,
                &thread.concepts,
                &gossip_cfg,
            ...
            if deleted > 0 || created > 0 {
                tracing::info!(thread_id = %id, deleted = deleted, created = created, "Thinkbridges refreshed after label change");
            }

            Ok(serde_json::json!({"thread_id": id, "labels": thread.labels, "bridges_refreshed": {"deleted": deleted, "created": created}}))
        }
...
/// Annotation (#8): append a lightweight note to a thread.
/// No LLM, no extraction — just a plain text message appended to the thread.
/// Useful for marking threads as "obsolete", "superseded by X", etc.
pub fn handle_annotate(
    ...
    let thread = ThreadStorage::get(ctx.agent_conn, &thread_id)?
        .ok_or_else(|| ai_smartness::AiError::ThreadNotFound(thread_id.to_string()))?;

    // Append annotation as a message
    let msg = ThreadMessage {
        thread_id: thread_id.to_string(),
        msg_id: id_gen::message_id(),
        content: note.to_string(),
        source: "agent".to_string(),
        source_type: "annotation".to_string(),
        timestamp: time_utils::now(),
        metadata: serde_json::json!({"type": "annotation"}),
        is_truncated: false,
        continuity_from: None,
        continuity_to: None,
    };
    ...
    // Touch the thread's last_active to keep it fresh
    let mut updated = thread;
    updated.last_active = time_utils::now();
    ThreadStorage::update(ctx.agent_conn, &updated)?;
...
mod tests {
    ...
    fn test_create_rejects_at_quota() {
        ...
        assert!(result.is_ok(), "Should succeed after LRU eviction");
    }
