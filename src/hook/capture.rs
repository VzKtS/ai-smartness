...
//!
//! NOTE: Engram-on-Thinking (T14) injection moved to pretool.rs (PreToolUse)
//! because PostToolUse stdout is not visible to Claude (GitHub #18427).
//! PreToolUse `additionalContext` in `hookSpecificOutput` IS visible.
...
pub fn run(project_hash: &str, agent_id: &str, input: &str, session_id: Option<&str>) {
    let _ = session_id; // kept for API stability; engram injection moved to pretool.rs
    ...
    // 5. Clean content (ANSI strip, whitespace collapse) — LLM decides skip/extract
    let cleaned = cleaner::clean_tool_output(&output);
    if cleaner::is_junk(&cleaned) {
        tracing::info!(tool = %tool_name, cleaned_len = cleaned.len(), "Capture: filtered out (junk)");
        print_continue();
        return;
    }

    // 6. Extract reference + contextual header from tool_input
    //    The header gives the LLM the action context (command, file path, URL, query)
    //    so it can produce a meaningful summary, not just summarize raw output.
    let tool_input = data.get("tool_input");
    let (file_path, context_header) = extract_tool_context(tool_name, tool_input);

    // 7. Prepend context header to content so LLM sees: action + result
    let enriched = if let Some(header) = &context_header {
        format!("{}\n---\n{}", header, cleaned)
    } else {
        cleaned.clone()
    };

    // 8. Send to daemon via IPC (fire-and-forget, non-blocking)
    tracing::info!(tool = %tool_name, content_len = enriched.len(), file_path = ?file_path.as_deref(), "Capture sending to daemon");
    let _ = daemon_ipc_client::send_capture(project_hash, agent_id, tool_name, &enriched, file_path.as_deref());

    // 8. Always continue (hook must never block)
    print_continue();
}

/// Extract reference path and contextual header from tool_input.
///
/// Returns (file_path_for_thread, context_header_for_llm):
/// - Bash: command as header, no file_path
/// - Read/Edit/Write: file_path as both reference and header
/// - WebFetch: url as reference and header
/// - WebSearch: query as reference and header
/// - NotebookEdit: file_path as reference, cell info as header
/// - Task/Agent: no reference, task description as header if available
fn extract_tool_context(
    ...
    match tool_name {
        ...
        "Read" => {
            ...
            let header = fp.map(|p| format!("File: {}", p));
            (fp.map(String::from), header)
        }
        "Edit" | "Write" => {
            let fp = input.get("file_path").and_then(|v| v.as_str());
            let header = fp.map(|p| format!("File: {} (modified)", p));
            (fp.map(String::from), header)
        }
        "WebFetch" => {
            let url = input.get("url").and_then(|v| v.as_str());
            let header = url.map(|u| format!("URL: {}", u));
            (url.map(String::from), header)
        }
        "WebSearch" => {
            let query = input.get("query").and_then(|v| v.as_str());
            let header = query.map(|q| format!("Search: {}", q));
            (query.map(String::from), header)
        }
        "NotebookEdit" => {
            let fp = input.get("file_path").and_then(|v| v.as_str());
            let cell = input.get("cell_number").and_then(|v| v.as_u64());
            let header = match (fp, cell) {
                (Some(p), Some(c)) => Some(format!("Notebook: {} cell {}", p, c)),
                (Some(p), None) => Some(format!("Notebook: {}", p)),
                _ => None,
            };
            (fp.map(String::from), header)
        }
        "Task" | "Agent" => {
            ...
            let header = desc.map(|d| format!("Task: {}", d));
            (None, header)
        }
        _ => {
            // Unknown tool: try file_path, url, or command as fallback
            let fp = input.get("file_path").and_then(|v| v.as_str());
            (fp.map(String::from), None)
        }
    }
}
