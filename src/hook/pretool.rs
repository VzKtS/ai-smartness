...
pub fn run(project_hash: &str, agent_id: &str, input: &str, session_id: Option<&str>) {
    ...
    // Track tool call for observability (nanobeat counter) + session handoff action log
    record_tool_call(project_hash, agent_id, tool_name, &data);
...
/// Track tool call in BeatState for nanobeat observability + session handoff action log.
fn record_tool_call(project_hash: &str, agent_id: &str, tool_name: &str, data: &serde_json::Value) {
    ...
    // Session handoff: extract target from tool input for action tracking
    let target = extract_tool_target(tool_name, data);
    beat.record_action(tool_name, target.as_deref());

    beat.save(&agent_data);
}

/// Extract the most meaningful target identifier from a tool call's input.
fn extract_tool_target(tool_name: &str, data: &serde_json::Value) -> Option<String> {
    let input = data.get("tool_input")?;
    match tool_name {
        ...
        _ if tool_name.starts_with("mcp__ai-smartness__") => {
            // For MCP tools, extract the most identifying param
            let short_name = tool_name.strip_prefix("mcp__ai-smartness__").unwrap_or(tool_name);
            input.get("query")
                .or_else(|| input.get("thread_id"))
                .or_else(|| input.get("reason"))
                .and_then(|v| v.as_str())
                .map(|s| format!("{}:{}", short_name, if s.len() > 60 { &s[..60] } else { s }))
        }
        _ => None,
    }
}
