...
fn dispatch(
    ...
    match method {
        ...
        // Update PendingContext for an agent — called by MCP thread_create
        // so the next automatic capture chains continuity to the MCP-created thread.
        "update_pending_context" => {
            let key = extract_agent_key(params)?;
            let thread_id = params
                .get("thread_id")
                .and_then(|v| v.as_str())
            ...
            let pending_arc = pool.get_pending(&key)
                .map_err(|e| format!("Failed to get pending context: {}", e))?;
            let mut pending_guard = pending_arc.lock()
                .map_err(|e| format!("Pending context mutex poisoned: {}", e))?;

            *pending_guard = Some(PendingContext {
            ...
            tracing::info!(
                agent = %key.agent_id,
                thread_id = %thread_id,
                "PendingContext updated via IPC (MCP thread_create continuity)"
            );

            Ok(serde_json::json!({"updated": true, "thread_id": thread_id}))
        }

        // Get current PendingContext thread_id for an agent — used by MCP thread_create
        // to chain continuity when creating multiple threads in sequence.
        "get_pending_context" => {
            let key = extract_agent_key(params)?;
            let pending_arc = pool.get_pending(&key)
                .map_err(|e| format!("Failed to get pending context: {}", e))?;
            let pending_guard = pending_arc.lock()
                .map_err(|e| format!("Pending context mutex poisoned: {}", e))?;

            match pending_guard.as_ref() {
                Some(pc) => Ok(serde_json::json!({
