    ...
    if (projectHash) {
        ...
        try {
            ...
            if (cfg.auto_start) {
                ...
                if (!st.running) await invoke('daemon_start');
            }
        } catch (_) {}
    }
});
...
document.querySelectorAll('.tab:not(.tab-debug)').forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        if (tab.dataset.tab === 'graph') { loadGraph(); startGraphAutoRefresh(); }
        if (tab.dataset.tab !== 'graph') { stopGraphAnimation(); stopGraphAutoRefresh(); }
    });
});
...
let dashboardFocusedAgent = null;
let dashboardGlobalTotals = null;

function updateDashboardCards(agent) {
    if (agent) {
        document.getElementById('thread-active').textContent = agent.active || 0;
        document.getElementById('thread-suspended').textContent = agent.suspended || 0;
        document.getElementById('thread-archived').textContent = agent.archived || 0;
        document.getElementById('bridge-count').textContent = agent.bridges || 0;
    } else if (dashboardGlobalTotals) {
        const tc = dashboardGlobalTotals;
...
async function loadDashboard() {
    ...
    try {
        ...
        dashboardGlobalTotals = data.totals;
        overviewAgents = data.agents || [];

        // Update cards: show focused agent data or global totals
        if (dashboardFocusedAgent) {
            const agent = overviewAgents.find(a => a.id === dashboardFocusedAgent);
            updateDashboardCards(agent || null);
        } else {
...
// Click outside agent tree/detail to clear focus
document.getElementById('dashboard')?.addEventListener('click', (e) => {
    const hierarchy = document.getElementById('dashboard-hierarchy');
    const detailPanel = document.getElementById('agent-detail-panel');
    if (hierarchy?.contains(e.target) || detailPanel?.contains(e.target)) return;
    if (!dashboardFocusedAgent) return;
    dashboardFocusedAgent = null;
...
async function selectAgentInTree(selectedAgentId) {
    ...
    // 2. Set focus + update dashboard cards
    dashboardFocusedAgent = selectedAgentId;
...
function originLabel(type) {
    const map = { Prompt:'Prompt', FileRead:'File', FileWrite:'File', Response:'Response',
                  Task:'Task', Fetch:'Fetch', Command:'Command', Split:'Split', Reactivation:'Reactivation',
                  Agent:'Agent', Manual:'Agent', __mind__:'Mind' };
    return map[type] || type || '\u2014';
}
function originBadgeColor(type) {
    const colors = { FileRead:'#4a9eff', FileWrite:'#4a9eff', Prompt:'#fbbf24', Response:'#a78bfa',
                     Task:'#22d3ee', Fetch:'#f472b6', Command:'#94a3b8', Split:'#fb923c', Reactivation:'#86efac',
                     Agent:'#e879f9', Manual:'#e879f9', __mind__:'#ff6b6b' };
...
function applyOriginFilter() {
    ...
    if (origin === '__mind__') {
        ...
        filtered = currentThreads.filter(t => t.origin_type === 'FileRead' || t.origin_type === 'FileWrite');
    } else {
...
async function loadResources() {
    try {
        ...
        if (memEl && res.memory) {
            memEl.textContent = `RAM ${res.memory.percent != null ? res.memory.percent.toFixed(1) : '-'}% — ${res.memory.used_mb || 0} / ${res.memory.total_mb || 0} MB`;
        }

        // GPU card — show first GPU summary
        const gpuEl = document.getElementById('resource-gpu');
        const gpuVramEl = document.getElementById('resource-gpu-vram');
        try {
            const hw = await invoke('detect_hardware');
            if (gpuEl && hw.gpus && hw.gpus.length > 0) {
                const gpu = hw.gpus[0];
                gpuEl.textContent = (gpu.name || '-').replace(/^NVIDIA /, '');
                if (gpuVramEl && gpu.vram_total_mb > 0) {
                    const freeMb = gpu.vram_total_mb - gpu.vram_used_mb;
                    const pct = (gpu.vram_used_mb / gpu.vram_total_mb * 100).toFixed(1);
                    gpuVramEl.textContent = `VRAM ${pct}% — ${gpu.vram_used_mb}/${gpu.vram_total_mb} MB (${freeMb} free)`;
                } else if (gpuVramEl) {
                    gpuVramEl.textContent = '-';
                }
            } else {
                if (gpuEl) gpuEl.textContent = 'N/A';
                if (gpuVramEl) gpuVramEl.textContent = '';
            }
        } catch (_) {
...
function showDaemonSaveStatus(msg, isError) {
    const el = document.getElementById('daemon-save-status');
    if (!el) return;
    el.textContent = msg;
    el.className = 'save-status ' + (isError ? 'error' : 'success');
    setTimeout(() => { el.textContent = ''; el.className = 'save-status'; }, 4000);
...
async function saveDaemonSettings() {
    try {
        ...
        const result = await invoke('save_daemon_settings', { settings });
        if (result.saved) {
            showDaemonSaveStatus(T[currentLang]?.['settings.saved'] || 'Settings saved successfully');
        } else {
            showDaemonSaveStatus('Save failed', true);
        }
    } catch (e) {
        showDaemonSaveStatus('Error: ' + e, true);
    }
}
...
async function loadBackupHistory() {
    try {
        ...
        for (const b of backups) {
            ...
            tr.innerHTML = `<td>${b.date || '-'}</td><td>${b.agent_id}</td><td>${sizeMB} MB</td><td></td>`;
            const btnCell = tr.querySelector('td:last-child');
            const restoreBtn = document.createElement('button');
            restoreBtn.className = 'btn-sm';
            restoreBtn.title = 'Restore this backup to the agent\'s database';
            restoreBtn.textContent = 'Restore';
            restoreBtn.addEventListener('click', () => restoreBackup(b.path, b.agent_id));
            const deleteBtn = document.createElement('button');
            deleteBtn.className = 'btn-sm btn-danger';
            deleteBtn.title = 'Permanently delete this backup file';
            deleteBtn.textContent = 'Delete';
            deleteBtn.addEventListener('click', () => deleteBackup(b.path));
            btnCell.appendChild(restoreBtn);
...
// "See in Graph" — switch to graph tab and select the thread
document.getElementById('btn-see-in-graph')?.addEventListener('click', (e) => {
    e.preventDefault();
    if (!currentDetailThreadId || !currentDetailAgentId) return;
    const threadId = currentDetailThreadId;
    const agentId = currentDetailAgentId;
    // Close modal
    document.getElementById('modal-thread-detail').classList.remove('open');
    // Switch to graph tab
    ...
    // Set agent in graph selector
    const agentSel = document.getElementById('graph-agent-select');
    if (agentSel) agentSel.value = agentId;
    // Load graph then center on the thread
    loadGraph().then(() => {
        const node = graphNodes.find(n => n.id === threadId);
        if (node) {
...
// Initialize particles along all edges — preserves animation state for surviving edges
function initParticles() {
    // Save scalar state keyed by edge identity + particle index (handles multiple per edge)
    const oldState = {};
    for (const p of graphParticles) {
        const baseKey = p.edge.source + '|' + p.edge.target + '|' + p.edge.edge_type + '|' + p.dir;
        // Append incrementing index for duplicate keys (continuity has 2 forward particles)
        let idx = 0;
        while (oldState[baseKey + '|' + idx]) idx++;
        oldState[baseKey + '|' + idx] = { t: p.t, speed: p.speed };
    }
    ...
    for (const e of graphEdges) {
        ...
        for (let i = 0; i < count; i++) {
            const key = e.source + '|' + e.target + '|' + e.edge_type + '|1|' + i;
            const prev = oldState[key];
            graphParticles.push({
                edge: e,
                t: prev ? prev.t : Math.random(),
                speed: prev ? prev.speed : 0.002 + Math.random() * 0.002,
                dir: 1,
            });
        }
        if (e.edge_type !== 'continuity') {
            const key = e.source + '|' + e.target + '|' + e.edge_type + '|-1|0';
            const prev = oldState[key];
            graphParticles.push({
                edge: e,
                t: prev ? prev.t : Math.random(),
...
// Auto-refresh graph data every 5 seconds while the graph tab is active
let graphAutoRefreshInterval = null;
function startGraphAutoRefresh() {
    stopGraphAutoRefresh();
    graphAutoRefreshInterval = setInterval(() => { loadGraph(); }, 5000);
}
function stopGraphAutoRefresh() {
    if (graphAutoRefreshInterval) { clearInterval(graphAutoRefreshInterval); graphAutoRefreshInterval = null; }
}
...
async function loadGraph() {
    ...
    try {
        ...
        const isInitialLoad = graphNodes.length === 0;
        graphRawThreads = threads;
        graphRawBridges = liveBridges;
        graphRawContinuity = continuity || [];
        buildGraph(threads, liveBridges);
        // Re-link selected node to new object (buildGraph creates new array)
        if (graphSelectedNode) {
            graphSelectedNode = graphNodes.find(n => n.id === graphSelectedNode.id) || null;
        }
        initParticles();
        // Reset animation timestamp so the next frame sees dt≈0 (prevents particle jump after sync work)
        graphLastAnimTime = performance.now();
        if (isInitialLoad) {
            forceLayout(150);
            centerGraph();
        ...
        startGraphAnimation();
        // Update stats only if changed (avoid DOM reflow)
        ...
        const statsText = `${graphNodes.length} threads, ${bridgeCount} bridges, ${contCount} continuity`;
...
function buildGraph(threads, bridges) {
    ...
    let filtered = threads.filter(t => {
        ...
        if (fOrigin === '__mind__') {
            ...
            if (t.origin_type !== 'FileRead' && t.origin_type !== 'FileWrite') return false;
        } else if (fOrigin && (t.origin_type || '') !== fOrigin) return false;
        return true;
    });

    // Build nodes with enriched fields — preserve existing positions
    const oldPositions = {};
    for (const n of graphNodes) {
        oldPositions[n.id] = { x: n.x, y: n.y };
    }
    graphNodes = filtered.map(t => {
        const old = oldPositions[t.id];
        return {
            id: t.id,
...
function drawGraph() {
    ...
    // Find the most recently created thread for visual highlight
    let newestNodeId = null;
    if (graphNodes.length > 0) {
        let newest = graphNodes[0];
        for (const n of graphNodes) {
            if (n.created_at && n.created_at > (newest.created_at || '')) newest = n;
        }
        newestNodeId = newest.id;
    }

    // Compute connected node IDs + glow color for selected node highlight
    const selectedConnectedColors = {};
    if (graphSelectedNode) {
        for (const e of graphEdges) {
            const otherId = e.source === graphSelectedNode.id ? e.target
                          : e.target === graphSelectedNode.id ? e.source : null;
            if (!otherId) continue;
            // Continuity → violet, bridges → relation color
            const color = e.edge_type === 'continuity'
                ? 'rgba(167,139,250,0.9)'
                : (RELATION_COLORS[e.relation] || GRAPH_COLORS.edge_default);
            selectedConnectedColors[otherId] = color;
        }
    }
    ...
    for (const n of graphNodes) {
        ...
        // Newest thread: white luminous glow
        const isNewest = n.id === newestNodeId;
        if (isNewest && !isDimmed) {
            ctx.shadowColor = '#ffffff';
            ctx.shadowBlur = 14 * scale;
            ctx.beginPath();
            ctx.arc(nx, ny, r + Math.max(3, 4 * scale), 0, Math.PI * 2);
        ...
        // Selected node: blue luminous glow
        ...
        // Connected to selected node: glow color matches edge type
        const connColor = selectedConnectedColors[n.id];
        if (!isSelected && !isDimmed && connColor) {
...
function showGraphDetail(node) {
    ...
    const conf = node.confidence !== undefined ? node.confidence : '-';
    meta += `<strong>Weight:</strong> ${node.weight.toFixed(2)} &nbsp; <strong>Importance:</strong> ${node.importance.toFixed(2)} &nbsp; <strong>Confidence:</strong> ${typeof conf === 'number' ? conf.toFixed(2) : conf}<br>`;
    ...
    // Bridges + Continuity sections (separated)
    const connected = graphEdges.filter(e => e.source === node.id || e.target === node.id);
    const bridges = connected.filter(e => e.edge_type !== 'continuity');
    const continuityEdges = connected.filter(e => e.edge_type === 'continuity');
    const nodeMap = {};
    graphNodes.forEach(n => { nodeMap[n.id] = n; });
    const fmtEdge = (e) => {
        ...
        return `<span style="color:${RELATION_COLORS[e.relation] || GRAPH_COLORS.info}">${esc(e.relation)}</span> → ${esc(otherTitle)} <span style="color:var(--text-dim)">(w:${e.weight.toFixed(2)} c:${e.confidence.toFixed(2)})</span>`;
    };
    let edgesHtml = `<strong>Bridges (${bridges.length}):</strong><br>`;
    edgesHtml += bridges.length > 0 ? bridges.map(fmtEdge).join('<br>') : '<span style="color:var(--text-dim)">None</span>';
    if (continuityEdges.length > 0) {
        edgesHtml += `<br><br><strong>Continuity (${continuityEdges.length}):</strong><br>`;
        edgesHtml += continuityEdges.map(fmtEdge).join('<br>');
    }
    document.getElementById('graph-detail-bridges').innerHTML = edgesHtml;
}

// Delete thread from graph detail panel
document.getElementById('btn-graph-detail-delete')?.addEventListener('click', async () => {
    if (!graphSelectedNode) return;
    const aid = document.getElementById('graph-agent-select')?.value;
    if (!aid || !projectHash) return;
    const tid = graphSelectedNode.id;
    const title = graphSelectedNode.title;
    if (!confirm(`Delete thread "${title}" and all associated bridges?`)) return;
    try {
        const result = await invoke('delete_thread', { projectHash, agentId: aid, threadId: tid });
...
// "See more" — navigate to Threads tab and open thread detail
document.getElementById('btn-graph-detail-seemore')?.addEventListener('click', (e) => {
    e.preventDefault();
    if (!graphSelectedNode) return;
    const aid = document.getElementById('graph-agent-select')?.value;
    if (!aid) return;
    ...
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.panel').forEach(p => p.classList.remove('active'));
    const threadsTab = document.querySelector('.tab[data-tab="threads"]');
    if (threadsTab) threadsTab.classList.add('active');
    document.getElementById('threads')?.classList.add('active');
    stopGraphAnimation();
    stopGraphAutoRefresh();
    // Open thread detail modal
    openThreadDetail(graphSelectedNode.id, aid);
    graphSelectedNode = null;
});

// Click outside graph detail panel to close it
...
document.getElementById('btn-graph-reorganise')?.addEventListener('click', () => {
    // Reset all positions to random so force layout starts fresh
    for (const n of graphNodes) {
...
document.getElementById('btn-graph-filter-reset')?.addEventListener('click', () => {
    ...
    try { localStorage.removeItem(GRAPH_PREFS_KEY); } catch (_) {}
    applyGraphFilters();
});

// Graph preferences persistence (localStorage)
const GRAPH_PREFS_KEY = 'ai-smartness-graph-prefs';
const GRAPH_PREF_IDS = [
    { id: 'graph-show-labels', type: 'checkbox' },
    { id: 'graph-show-weights', type: 'checkbox' },
    { id: 'graph-show-legend', type: 'checkbox' },
    { id: 'graph-f-active', type: 'checkbox' },
    { id: 'graph-f-suspended', type: 'checkbox' },
    { id: 'graph-f-archived', type: 'checkbox' },
    { id: 'graph-f-continuity', type: 'checkbox' },
    { id: 'graph-f-imp', type: 'range' },
    { id: 'graph-f-bridges', type: 'number' },
    { id: 'graph-f-origin', type: 'select' },
    { id: 'graph-spacing', type: 'range' },
];

function saveGraphPrefs() {
    const prefs = {};
    for (const p of GRAPH_PREF_IDS) {
        const el = document.getElementById(p.id);
        if (!el) continue;
        prefs[p.id] = p.type === 'checkbox' ? el.checked : el.value;
    }
...
function loadGraphPrefs() {
    try {
        ...
        for (const p of GRAPH_PREF_IDS) {
            if (!(p.id in prefs)) continue;
            const el = document.getElementById(p.id);
            if (!el) continue;
            if (p.type === 'checkbox') {
                el.checked = prefs[p.id];
            } else {
                el.value = prefs[p.id];
            }
        }
        // Sync derived UI
        const impVal = parseInt(document.getElementById('graph-f-imp')?.value || '0');
        document.getElementById('graph-f-imp-label').textContent = (impVal / 100).toFixed(2);
        const spacingVal = parseInt(document.getElementById('graph-spacing')?.value || '3000');
        document.getElementById('graph-spacing-label').textContent = spacingVal >= 1000 ? (spacingVal / 1000) + 'k' : spacingVal;
        graphSpacing = spacingVal / 3000;
    } catch (_) {}
}
...
// Save prefs on any change
for (const p of GRAPH_PREF_IDS) {
    const el = document.getElementById(p.id);
    if (!el) continue;
    el.addEventListener('change', saveGraphPrefs);
    if (p.type === 'range') el.addEventListener('input', saveGraphPrefs);
}
...
function renderGraphLegend() {
    ...
    if (activeOrigins.size > 0) {
        ...
        const originOrder = ['FileRead', 'Prompt', 'Response', 'Command', 'Task', 'Fetch', '__mind__'];
        const shownLabels = new Set();
        for (const o of originOrder) {
            // FileRead/FileWrite both map to "File" — show once
            if (o === 'FileRead' && !activeOrigins.has('FileRead') && !activeOrigins.has('FileWrite')) continue;
            if (!activeOrigins.has(o) && o !== 'FileRead') continue;
            const label = originLabel(o);
            if (shownLabels.has(label)) continue;
            shownLabels.add(label);
            html += `<span style="color:${originBadgeColor(o)}">◯</span> ${label} &nbsp; `;
        }
    }
    html += `<br><span style="color:${GRAPH_COLORS.text_dim}">— Highlights —</span><br>`;
    const glowCircle = (color) => `<span style="display:inline-block;width:12px;height:12px;border-radius:50%;border:2px solid ${color};box-shadow:0 0 6px ${color};vertical-align:middle;margin-right:3px"></span>`;
    html += `${glowCircle('rgba(255,255,255,0.9)')} Latest thread &nbsp; `;
    html += `${glowCircle('rgba(79,195,247,0.9)')} Selected &nbsp; `;
    html += `${glowCircle('rgba(167,139,250,0.9)')} Connected (continuity) &nbsp; `;
    html += `${glowCircle(RELATION_COLORS['Sibling'] || '#40d0a0')} Connected (bridge color)`;
