...
pub fn migrate_agent_db(conn: &Connection) -> AiResult<()> {
    ...
    // V10: add shard_concept to bridges (per-shard ThinkBridge model)
    if version < 10 {
        conn.execute_batch(
            "ALTER TABLE bridges ADD COLUMN shard_concept TEXT;
             CREATE INDEX IF NOT EXISTS idx_bridges_shard ON bridges(shard_concept);"
        ).map_err(|e| AiError::Storage(format!("Agent DB V10 migration failed: {}", e)))?;
        set_schema_version(conn, 10)?;
    }

    // V11: add confidence column to threads (weight rework: dynamic weight = (importance + confidence) / 10)
    if version < 11 {
        conn.execute_batch(
            "ALTER TABLE threads ADD COLUMN confidence REAL DEFAULT 0.5;"
        ).map_err(|e| AiError::Storage(format!("Agent DB V11 migration failed: {}", e)))?;
