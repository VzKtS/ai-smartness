...
pub fn find_threads_sharing_concepts_db(
    ...
    // Exclude Command (Bash) threads from bridge candidates — they get continuity only
    let mut stmt = conn.prepare(
        "SELECT id, concepts FROM threads WHERE status IN ('Active', 'Suspended') AND concepts != '[]' AND origin_type != 'command'"
...
impl ConceptIndex {
    ...
    pub fn build_from_db(conn: &Connection) -> AiResult<Self> {
        ...
        // Exclude Command (Bash) threads — they pollute the concept graph with build noise
        let mut stmt = match conn.prepare(
            "SELECT id, concepts FROM threads WHERE concepts != '[]' AND origin_type != 'command'"
        ) {
            Ok(s) => s,
            Err(_) => return Ok(idx),
        };
