...
use crate::constants::ARCHIVE_AFTER_HOURS;
...
impl Archiver {
    /// Archive suspended threads inactive for > 72 hours.
    /// Returns count of archived threads.
    pub fn archive_stale(conn: &Connection) -> AiResult<u32> {
        let now = Utc::now();
        ...
        for thread in &suspended {
            ...
            if hours_inactive >= ARCHIVE_AFTER_HOURS {
