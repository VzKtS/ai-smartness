...
impl EngramRetriever {
    ...
    pub fn get_relevant_context(
        ...
        let candidates = filter_engram_candidates(load_threads_by_ids(conn, &candidate_ids)?);
        ...
        // === Phase 2.5: Min bridge connections filter ===
        // Threads with fewer than min_bridge_connections are excluded from injection.
        // Aligns with GUI "Min Bridges" display filter — same threshold, same behavior.
        let min_bridges = self.config.min_bridge_connections;
        if min_bridges > 0 {
            let bridge_counts = load_bridge_counts(conn, &scores)?;
            let before = scores.len();
            scores.retain(|s| bridge_counts.get(&s.thread_id).copied().unwrap_or(0) >= min_bridges);
            let filtered = before - scores.len();
            if filtered > 0 {
                tracing::debug!(filtered, min_bridges, "Engram: min_bridge_connections filter");
            }
        }
        ...
        // Continuity expansion: for each injected thread, pull in its
        // continuity parent and children so the agent sees the full workflow.
        let result = expand_continuity_neighbors(conn, result);
    ...
    pub fn search(
        ...
        let scored_result: Vec<ScoredThread> = scores.iter()
            .take(limit)
            .filter_map(|s| candidates.iter().find(|t| t.id == s.thread_id).map(|t| ScoredThread {
                thread: t.clone(),
                weighted_score: s.weighted_score,
                pass_count: s.pass_count,
                decision: s.decision.clone(),
        ...
        // Continuity expansion: pull in parent/children for workflow visibility
        let scored_result = expand_continuity_neighbors(conn, scored_result);

        let result: Vec<Thread> = scored_result.into_iter().map(|st| st.thread).collect();
        Ok(result)
    }
...
/// Continuity expansion: for each injected thread, include its continuity parent
/// and continuity children so the agent sees the full workflow chain.
/// Neighbors are appended with WeakInject if not already present.
fn expand_continuity_neighbors(conn: &Connection, mut result: Vec<ScoredThread>) -> Vec<ScoredThread> {
    use crate::storage::threads::ThreadStorage;
    use std::collections::HashSet;

    let existing_ids: HashSet<String> = result.iter().map(|st| st.thread.id.clone()).collect();
    let mut neighbors_to_add: Vec<Thread> = Vec::new();
    let mut added_ids: HashSet<String> = HashSet::new();

    for st in &result {
        // Continuity parent
        if let Some(ref parent_id) = st.thread.continuity_parent_id {
            if !existing_ids.contains(parent_id) && !added_ids.contains(parent_id) {
                if let Ok(Some(parent)) = ThreadStorage::get(conn, parent_id) {
                    added_ids.insert(parent.id.clone());
                    neighbors_to_add.push(parent);
                }
            }
        }
        // Continuity children: threads whose continuity_parent_id = this thread's id
        if let Ok(children) = find_continuity_children(conn, &st.thread.id) {
            for child in children {
                if !existing_ids.contains(&child.id) && !added_ids.contains(&child.id) {
                    added_ids.insert(child.id.clone());
                    neighbors_to_add.push(child);
                }
            }
        }
    }
    ...
    if !neighbors_to_add.is_empty() {
        tracing::debug!(
            count = neighbors_to_add.len(),
            "Engram: expanding continuity neighbors"
        );
        for thread in neighbors_to_add {
            result.push(ScoredThread {
                thread,
                weighted_score: 0.1, // Low score — context only
                pass_count: 1,
                decision: InjectionDecision::WeakInject,
            });
        }
    }
...
/// Find threads whose continuity_parent_id points to the given thread.
fn find_continuity_children(conn: &Connection, thread_id: &str) -> AiResult<Vec<Thread>> {
    use crate::storage::threads::ThreadStorage;

    let child_ids: Vec<String> = conn
        .prepare("SELECT id FROM threads WHERE continuity_parent_id = ?1")
        .and_then(|mut stmt| {
    ...
    for id in &child_ids {
        if let Ok(Some(t)) = ThreadStorage::get(conn, id) {
            children.push(t);
        }
    }
    Ok(children)
...
/// Filter engram candidates to injectable origin types only.
/// Injected: Prompt, Response, Agent (incl. __mind__), Split, Reactivation.
/// Excluded: FileRead, FileWrite, Command, Task, Fetch — these are
/// continuity-only anchors, accessible via ai_recall but not auto-injected.
fn filter_engram_candidates(candidates: Vec<Thread>) -> Vec<Thread> {
    candidates.into_iter().filter(|t| matches!(
        t.origin_type,
        OriginType::Prompt | OriginType::Response | OriginType::Agent
        | OriginType::Split | OriginType::Reactivation
    )).collect()
}
...
fn load_threads_by_ids(
    ...
    let sql = format!(
        "SELECT id, title, status, weight, importance, importance_manually_set, \
         ...
         has_truncated_origin, continuity_parent_id, subject_coherence, confidence \
...
fn row_to_thread(row: &rusqlite::Row) -> rusqlite::Result<Thread> {
    ...
    Ok(Thread {
        ...
        confidence: row.get(28).unwrap_or(0.5),
    })
}
...
/// Returns a map of connected thread_id → aggregate bridge weight (sum, capped at 1.0).
/// Per-shard model: more bridges between a pair = stronger connection.
fn load_bridge_connections(
    ...
    // Cap at 1.0 — sum of shard weights naturally saturates
    for weight in connections.values_mut() {
...
/// Load bridge counts for scored candidates (batch query).
/// Returns thread_id → total bridge count (excluding continuity).
fn load_bridge_counts(
    conn: &Connection,
    scores: &[EngramScore],
) -> AiResult<HashMap<String, usize>> {
    let mut counts = HashMap::new();
    for s in scores {
        let count = BridgeStorage::count_connected_threads(conn, &s.thread_id)?;
        counts.insert(s.thread_id.clone(), count);
    }
    Ok(counts)
}
