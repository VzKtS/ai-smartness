...
pub fn extract(
    ...
    for attempt in 0..3u8 {
        match extract_via_llm(content, source, extraction_cfg, label_cfg, importance_cfg, agent_context, model, min_capture_length) {
...
fn extract_via_llm(
    ...
    let prompt = build_extraction_prompt(content, source, extraction_cfg, label_cfg, importance_cfg, agent_context, model, min_capture_length)?;
    tracing::info!(
...
Higher alignment = higher importance. No alignment does NOT mean low importance — content may be independently valuable."#,
    ...
    Ok(template
        ...
        .replace("{min_capture_length}", &min_capture_length.to_string())
...
fn parse_extraction_response(response: &str) -> AiResult<ExtractionResult> {
    // Fix double-brace issue: TOML templates use `{{` to escape `{`, but the LLM
    // sometimes reproduces `{{`/`}}` in its output. Sanitize before parsing.
    let sanitized: std::borrow::Cow<'_, str> = if response.contains("{{") || response.contains("}}") {
        tracing::debug!("Sanitizing double braces in LLM response");
    ...
    let json_str_opt = extract_first_json_object(&sanitized);
