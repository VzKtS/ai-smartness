...
mod tests {
    ...
    fn test_model_dir_names() {
        ...
        // default() maps to SevenB (with Phi4Mini as serde alias)
        assert_eq!(model_dir_name(&LocalModelSize::default()), "Qwen-7B");
    }
    ...
    #[test]
    fn test_load_default_extractor() {
        // This test works when run from the project root (cargo test)
        if let Ok(tmpl) = load_prompt(&LocalModelSize::default(), PromptName::Extractor) {
            assert_eq!(tmpl.meta.version, 2);
    ...
    #[test]
    fn test_load_default_toolextractor() {
        if let Ok(tmpl) = load_prompt(&LocalModelSize::default(), PromptName::ToolExtractor) {
            assert_eq!(tmpl.meta.version, 3);
            assert!(tmpl.template.prompt.contains("{content}"));
    ...
    fn test_cache_returns_same_instance() {
        clear_cache();
        if let Ok(t1) = load_prompt(&LocalModelSize::default(), PromptName::Extractor) {
            let t2 = load_prompt(&LocalModelSize::default(), PromptName::Extractor).unwrap();
