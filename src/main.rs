...
enum Commands {
    ...
    /// Run custom agent runtime (direct Anthropic API, push-not-pull memory)
    Runtime {
        /// Project hash (auto-detected from current dir if omitted)
        #[arg(long)]
        project_hash: Option<String>,
        /// Agent ID
        #[arg(long)]
...
enum ModelAction {
    ...
    Info {
        /// Model name (qwen7b, gemma12b, qwen14b, qwen32b)
        name: String,
    },
...
fn main() {
    ...
    match app.command {
        ...
        // Runtime: custom agent runtime (direct Anthropic API)
        Some(Commands::Runtime { project_hash, agent_id, model }) => {
            let ph = cli::resolve_project_hash(project_hash.as_deref())
                .unwrap_or_else(|e| { eprintln!("Error: {}", e); std::process::exit(1); });
            let aid = cli::resolve_agent_id(agent_id.as_deref(), &ph)
                .unwrap_or_else(|e| { eprintln!("Error: {}", e); std::process::exit(1); });
            if let Some(m) = model {
                std::env::set_var("AI_SMARTNESS_RUNTIME_MODEL", &m);
            }
            runtime::run(&ph, &aid);
        }
