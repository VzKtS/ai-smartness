...
pub enum OriginType {
    ...
    Reactivation,
    /// Created by agent via MCP tool (ai_thread_create)
    Agent,
}
...
impl OriginType {
    ...
    pub fn compatibility_group(&self) -> &'static str {
        match self {
            ...
            Self::Task => "task",
            Self::Fetch => "web",
            Self::Split | Self::Reactivation | Self::Agent => "system",
        }
    }
...
impl std::str::FromStr for OriginType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "agent" | "Agent" | "manual" | "Manual" => Ok(Self::Agent),
...
pub struct Thread {
    ...
    /// LLM extraction confidence (0.0-1.0). Used in dynamic weight: (importance + confidence) / 10.
